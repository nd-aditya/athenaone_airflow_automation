...
from .phi_validation_tools import PHIValidationToolsManager, build_mapping_cache, init_worker_mapping_cache, is_mapping_cache_loaded, init_mapping_cache
...
class PHIDeidentificationPipelineOptimized:
    ...
    def __init__(self, config_path: Optional[str] = None, json_dir: Optional[str] = None):
        try:
            ...
            self.master_db_manager = DatabaseManager(self.config_manager.get_mapping_config())
    ...
    def _ensure_mapping_cache_built(self) -> None:
        ...
        with _CACHE_BUILD_LOCK:
            ...
            try:
                _MAIN_PROCESS_MAPPING_CACHE = build_mapping_cache(self.config, self.master_db_manager)
    ...
    # Validat\ion pool initialization removed - now handled by consumer process
    ...
    def _cleanup(self) -> None:
        ...
        try:
            ...
            if self.master_db_manager:
                try:
                    self.master_db_manager.close_connection()
                except Exception as e:
            ...
            if classification_result.is_phi == 'yes' and classification_result.phi_type:
                try:
                    ...
                    if self.validation_manager is None:
                        ...
                        self.validation_manager = PHIValidationToolsManager(self.config, self.db_manager, self.master_db_manager)
