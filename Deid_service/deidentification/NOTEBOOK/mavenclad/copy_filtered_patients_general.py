...
# Example Engines (pick one source)
# engine = create_engine("mssql+pymssql://sa:ndADMIN2025@localhost:1433/db_masnin")
engine = create_engine("mysql+pymysql://user:pass@host:3306/source_db")
...
# === STEP 2: Load tables and columns from SOURCE ===
try:
...
# === STEP 3: Map SQL Server types to MySQL (used only for MSSQL source) ===
def map_sqlserver_to_mysql(sql_type, max_len, precision, scale, observed_max_length=None):
    ...
    if sql_type in ["int", "smallint", "bigint", "tinyint"]:
        ...
        return "TINYINT(1)"
    elif sql_type in ["datetime", "smalldatetime", "date", "time", "datetime2", "timestamp"]:
        return "DATETIME"
...
patient_id_keywords = [
    ...
    "ecwpid", "associateduid", "localpid", "urefid", "uid", "userid",
    "contactid", "users_uid", "patientControlNo", "ControlNo"
]
...
encounter_id_keywords = [
    ...
    "refencid", "externalencounterid", "linkedencounterid", "grpencid"
]
...
for (schema, table), group in columns_df.groupby(["schema_name", "table_name"]):
    fq_table = f"{schema}.{table}" if dialect == "mssql" else f"`{schema}`.`{table}`"
    ...
    try:
        with mpengine.begin() as mconn:
            ...
            for _, col in group.iterrows():
                ...
                else:  # MySQL → MySQL: reuse original type
                    if col["data_type"].lower() in ["varchar", "char"]:
                        if col["max_length"]:
                            data_type = f"{col['data_type'].upper()}({int(col['max_length'])})"
                        else:
                            data_type = "TEXT"
                    elif col["data_type"].lower() in ["text", "longtext"]:
                        data_type = "LONGTEXT"
                    elif col["data_type"].lower() in ["blob", "longblob"]:
                        data_type = "LONGBLOB"
                    else:
