"""
GCP Deid merge from a staging schema into DEIDENTIFIED_SCHEMA.

Implements the same 3-phase merge behavior as `services/deid_merge_service.py`:
1) Phase insert:
   - set nd_active_flag='N' for rows whose PK exists in `staging_deid_schema`
   - insert new rows from `staging_deid_schema` into `DEIDENTIFIED_SCHEMA` with nd_active_flag='Y'
2) Phase validate: one active row per PK
3) Phase fix: set all N then one Y per PK (latest by LASTUPDATED / nd_auto_increment_id)

This file is intentionally written to not hardcode secrets. It expects DB
credentials via environment variables on the Airflow worker (or wherever tasks run).
"""
...
# ---- Config: taken from environment to avoid committing secrets ----
...
TEXT_PREFIX_LEN = int(os.getenv("TEXT_PREFIX_LEN", "255"))
MERGE_DEID_MAX_WORKERS = int(os.getenv("MERGE_DEID_MAX_WORKERS", "10"))
SET_DEID_FLAGS_MAX_WORKERS = int(os.getenv("SET_DEID_FLAGS_MAX_WORKERS", "8"))
VALIDATION_DEID_MAX_WORKERS = int(os.getenv("VALIDATION_DEID_MAX_WORKERS", "8"))
FIX_DEID_MAX_WORKERS = int(os.getenv("FIX_DEID_MAX_WORKERS", "8"))

TEMP_DEID_PK_PREFIX = os.getenv("TEMP_DEID_PK_PREFIX", "tmp_deid_pks_")

# Sits alongside this service file in GCP_related/airflow_home/services/
TABLE_PRIMARY_KEYS_CSV = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "table_primary_keys.csv")
)
...
def _require_env() -> None:
    missing = [k for k, v in {"MYSQL_USER": MYSQL_USER, "MYSQL_PASSWORD": MYSQL_PASSWORD, "MYSQL_HOST": MYSQL_HOST}.items() if not v]
    if missing:
        raise RuntimeError(
            "Missing required DB env vars for gcp_deid_merge_from_staging_service: "
            + ", ".join(missing)
            + ". Set MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD on the Airflow worker."
        )
...
def _deid_merge_engine():
    _require_env()
    connection_str = f"mysql+pymysql://{MYSQL_USER}:{MYSQL_PASSWORD}@{MYSQL_HOST}:{MYSQL_PORT}"
...
def _build_index_columns(conn, schema: str, table: str, cols: list) -> list:
    ...
    return [f"`{c}`({TEXT_PREFIX_LEN})" if c in text_cols else f"`{c}`" for c in cols]
...
def _set_deid_merged_flags_to_n_one_table(engine, hist_tbl: str, incr_tbl: str, pk_cols: list, deid_schema: str) -> dict:
    """
    For one table: create temp table of PKs from deid_schema, UPDATE merged SET nd_active_flag = 'N'
    only where PK in that temp, then drop temp.
    """
...
def set_deid_merged_flags_to_n(engine, deid_schema: str, common_tasks: list[tuple[str, str, list]]) -> list[dict]:
    """
    ...
    with ThreadPoolExecutor(max_workers=SET_DEID_FLAGS_MAX_WORKERS) as executor:
        future_to_hist = {
            executor.submit(_set_deid_merged_flags_to_n_one_table, engine, hist_tbl, incr_tbl, pk_cols, deid_schema): hist_tbl
...
def _merge_table(engine, hist_tbl: str, incr_tbl: str, pk_cols: list, incr_schema: str) -> dict:
    ...
    try:
        with engine.begin() as conn:
            ...
            cols = [c["name"] for c in inspect(engine).get_columns(hist_tbl, schema=DEIDENTIFIED_SCHEMA) if c["name"] != "nd_active_flag"]
...
def _validate_deid_table_one_active_per_pk(engine, table_name: str, pk_cols: list) -> dict:
...
def validate_deid_merged_one_active_per_pk(engine, merge_results: list[dict], pk_map: dict) -> dict:
    """
    Validate tables merged in phase 1: one active row per PK.
    Returns summary with validation_failed_tables.
    """
    tables_to_validate = [
        r["table"]
        for r in merge_results
        if r.get("status") == "SUCCESS" and (r.get("inserted", 0) > 0 or r.get("inserted") is None)
    ]
    results: list[dict] = []
    validation_failed_tables: list[dict] = []
    ...
    with ThreadPoolExecutor(max_workers=VALIDATION_DEID_MAX_WORKERS) as executor:
        ...
        for table_name in tables_to_validate:
            ...
            if not pk_cols:
                ...
                continue
            future_to_table[
                executor.submit(_validate_deid_table_one_active_per_pk, engine, table_name, pk_cols)
            ] = table_name
...
def _fix_deid_merged_one_table(engine, table_name: str, pk_cols: list) -> dict:
    """Fix one table: SET all to 'N', then SET 'Y' for one row per PK (latest)."""
    hist_fqn = f"{_q(DEIDENTIFIED_SCHEMA)}.{_q(table_name)}"
...
def fix_deid_merged_one_active_per_pk(engine, validation_summary: Optional[dict] = None) -> dict:
    """Fix nd_active_flag for tables that failed validation."""
    ...
    return {
        "fixed_tables": fixed,
        "skipped": len(failed_list) - fixed - failed,
        "failed": failed,
        "per_table": per_table,
    }


# ----------------- Phase APIs: what DAG calls -----------------

def merge_deid_staging_to_merged_phase_insert(staging_deid_schema: str) -> dict:
    """
    Phase 1:
    - set DEIDENTIFIED_SCHEMA.nd_active_flag='N' for PKs present in staging_deid_schema
    - INSERT rows from staging_deid_schema into DEIDENTIFIED_SCHEMA with nd_active_flag='Y'
    """
    engine = _deid_merge_engine()
    try:
        pk_map = _load_pk_config()
        table_map, incr_only = _get_table_map(engine, staging_deid_schema)

        results: list[dict] = []
        merge_tasks: list[tuple[str, str, list]] = []
        common_tasks: list[tuple[str, str, list]] = []

        for logical, (hist_tbl, incr_tbl) in table_map.items():
            if logical not in pk_map:
                results.append({"table": hist_tbl, "status": "SKIPPED", "duration": 0, "logs": ["No primary key in CSV"]})
                continue
        ...
        for logical, incr_tbl in incr_only.items():
            if logical not in pk_map:
                results.append({"table": incr_tbl, "status": "SKIPPED", "duration": 0, "logs": ["No primary key in CSV"]})
                continue
            try:
                _create_table_from_deid(engine, staging_deid_schema, incr_tbl)
                merge_tasks.append((incr_tbl, incr_tbl, pk_map[logical]))
            except Exception as e:
                results.append({"table": incr_tbl, "status": "FAILED", "duration": 0, "logs": [f"Create table failed: {e}"]})

        set_flags_results = set_deid_merged_flags_to_n(engine, staging_deid_schema, common_tasks)

        with ThreadPoolExecutor(max_workers=MERGE_DEID_MAX_WORKERS) as executor:
            future_to_hist = {
                executor.submit(_merge_table, engine, hist_tbl, incr_tbl, pk_cols, staging_deid_schema): hist_tbl
            ...
            for future in as_completed(future_to_hist):
                ...
                try:
                    ...
                    results.append({"table": hist_tbl, "status": "FAILED", "duration": 0, "logs": [f"ERROR: {e}"], "inserted": 0})
        ...
        return {
            "staging_deid_schema": staging_deid_schema,
...
def merge_deid_staging_to_merged_phase_validate(merge_insert_summary: dict) -> dict:
    """Phase 2: validate one active row per PK."""
    ...
    try:
        pk_map = _load_pk_config()
        return validate_deid_merged_one_active_per_pk(engine, merge_insert_summary["per_table"], pk_map)
    finally:
        engine.dispose()


def merge_deid_staging_to_merged_phase_fix(validation_summary: dict) -> dict:
    """Phase 3: fix tables that failed validation."""
    engine = _deid_merge_engine()
    try:
