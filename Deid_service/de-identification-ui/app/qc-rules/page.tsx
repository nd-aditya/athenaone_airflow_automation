...
export default function QCRules() {
  ...
  const loadConfig = async () => {
    setLoading(true)
    try {
      const response = await configApi.getQCConfig()
      if (response.success && response.qc_config) {
        setConfig(JSON.stringify(response.qc_config, null, 2))
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      setSaving(true)
      setMessage(null)
      const response = await configApi.saveQCConfig({ qc_config: parsed })
      if (response.success) {
    ...
    <Layout>
      <div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-6">QC Rules Configuration</h1>
